 ...
 * must be numbered using sequential integers , so we don't need to search for a node in the
 ...
 *
 * @author davebaol */
public class MyIndexAStarPathFinder<N> implements PathFinder<N> {
    ...
    public MyIndexAStarPathFinder(IndexedGraph<N> graph) {
        this(graph, false);
    }

    @SuppressWarnings("unchecked")
    public MyIndexAStarPathFinder(IndexedGraph<N> graph, boolean calculateMetrics) {
        this.graph = graph;
    ...
    protected void search (N startNode, N endNode, Heuristic<N> heuristic) {
        ...
        do {
            ...
            if (current.node.equals(endNode)) return;
    ...
    public boolean search (PathFinderRequest<N> request, long timeToRun) {
        ...
        do {
            ...
            if (current.node.equals(request.endNode)) {
    ...
    /** A class used by {@link MyIndexAStarPathFinder} to collect search metrics.
     *
     * @author davebaol */
